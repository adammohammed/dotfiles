...
if [ "${TERM%%-*}" = 'linux' ]; then
    # This script doesn't support linux console (use 'vconsole' template
    # instead)
...
if [ -n "$TMUX" ]; then
    ...
    # GNU screen (screen, screen-256color,
    # screen-256color-bce)
...
if [ -n "$ITERM_SESSION_ID" ]; then
    ...
    printf $printf_template_custom Pg D0D0D0
    # forground
    printf $printf_template_custom Ph
    151515 # background
    printf $printf_template_custom Pi
    D0D0D0 # bold color
    printf $printf_template_custom Pj
    303030 # selection color
    printf $printf_template_custom
    Pk D0D0D0 # selected text color
    printf $printf_template_custom
    Pl D0D0D0 # cursor
    printf
    $printf_template_custom Pm
    151515 # cursor text
else
    printf
    $printf_template_var 10
    $color_foreground
    printf
    $printf_template_var 11
    $color_background
    printf
    $printf_template_var
    12 $color_cursor
